...
```{r setup}
library(tidyverse)
library(stringr)          # (no mention in video, but "Model" section needs it for `str_detect`)
```
...
```{r}
houston <- permits %>% filter(str_detect(area, "Houston"))  # (`str_detect` needs **stringr**)
...
ggplot(houston, aes(month, f1units)) +                # to see seasonal effect
  geom_line(aes(group = year)) +
  scale_y_log10()                                     # (this isn't helpful)
```
...
* Houston in July is less pleasant than Houston in Dec

```{r}


# Now, I don't believe that this is a good model. I'm just using it as a way to partition the signal into months [15:04min]
